...
\matrix[array] (array) {
   1  &  2  &  3  &  4  &  5  & 6\\
  |[fill=gray!20]|4 &|[fill=gray!20]|1  &  &  & & \\
};

\node (Q) [left of=array-2-1, node distance=0.8cm] {Q};
\node (T) [below of=array-2-3, node distance=1.5cm] {$\texttt{Q.tail}=3$};
\node (H) [below of=array-2-1, node distance=1.2cm, xshift=0cm] {$\texttt{Q.head}=1$};
\draw[-latex] (T) to (array-2-3);
\draw[-latex] (H) to (array-2-1);
