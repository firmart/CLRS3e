...
\matrix[array] (array) {
   1  &  2  &  3  &  4  &  5  & 6\\
  4 & 1  &|[fill=gray!20]| 3  &|[fill=gray!20]| 8  & & \\
};

\node (Q) [left of=array-2-1, node distance=0.8cm] {Q};
\node (T) [below of=array-2-5, node distance=1.5cm] {$\texttt{Q.tail}=5$};
\node (H) [below of=array-2-3, node distance=1.2cm, xshift=0cm] {$\texttt{Q.head}=3$};
\draw[-latex] (T) to (array-2-5);
\draw[-latex] (H) to (array-2-3);
