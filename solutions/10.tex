...
\Procname{\textbf{Algorithme}\quad\textsc{Dequeue-2}$(\id{Q})$}
    ...
    \li \Return \id{e}
\end{codebox}
    \item \footnote{L'implémentation des algorithmes suivantes se trouve dans le dossier \pathx{src/CH10_Elementary_Data_Structures/Queue/Array_queue}.}
...
\begin{ex}
   On reprend les algorithmes \textsc{Enqueue-With-Counter} et \textsc{Dequeue-With-Counter} et les attribues définies. Les algorithmes suivantes complètent une file à une \textit{deque}.\footnote{L'implémentation des algorithmes suivantes se trouve dans le dossier \pathx{src/CH10_Elementary_Data_Structures/Queue/Array_deque/}.}
\begin{codebox}
...
\begin{description}
    \descitem{10.2-1} \textit{Can you implement the dynamic set operation \proc{Insert} on a singly linked list in $O(1)$ time ? How about \proc{Delete} ?}
    \descitem{10.2-2} \textit{Implement a stack using a singly linked list $L$. The operations \proc{Push} and \proc{Pop} should still take $O(1)$ time.}
    \begin{exrev}
      La seule possibilité d'implémenter une pile à partir d'une liste simplement chaînée tout en laissant les primitives \proc{Pop} et \proc{Push} en temps constant est de faire correspondre \attrib{S}{top} à \attrib{L}{head}. En effet, l'insertion et la suppression d'élément en tête dans une liste simplement chaînée se fait en $O(1)$. Si la liste est doublement circulaire, on peut faire correspondre \attrib{S}{top} au dernier élément de la liste (ce qui simplifie l'implémentation), avec les primitives en temps constant. 
    \end{exrev}
    \descitem{10.2-3} \textit{Implement a queue by a singly linked list $L$. The operations \proc{Enqueue} and \proc{Dequeue} should still take $O(1)$ time.}
    \descitem{10.2-4} \textit{As written, each loop iteration in the \proc{List-Search'} procedure requires two tests: one for $x \neq \attrib {L}{nil}$ and one for $\attrib{x}{key} \neq k$. Show how to eliminate the test for $x \neq \attrib{L}{nil}$ in each iteration.}
    \descitem{10.2-5} \textit{Implement the dictionary operations \proc{Insert}, \proc{Delete}, and \proc{Search} using singly linked, circular lists. What are the running times of your procedures?}
    \descitem{10.2-6} \textit{}
    \descitem{10.2-7} \textit{}
    \descitem{10.2-8} \textit{}
\end{description}
...
\begin{description}
    \descitem{10-1} \textit{\textbf{Comparisons among lists}}
...
\caption*{}
\end{longtable}
%TODO:add improve solution for min/max (by add a field) and separe doubly-linked list from XOR list
%TODO:clarifier le sens de successor/predecessor, pointer ou valeur ?
\end{pb}
\end{description}
