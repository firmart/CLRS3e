...
\left\{
	\begin{array}{ll}
	x_0 = \lceil y_0/2 \rceil\\
	x_i = \lceil y_i/2 \rceil
	\end{array}
  \right. ,
\end{matrix}
\]
o\`u $y_i$ d\'esigne le nombre de n\oe uds apr\`es avoir \^oter les n\oe uds de hauteur r\'espectives $0, \ldots, i-1$ et $x_i$ le nombre de feuille de l'arbre $i$.
\`A partir de ces relations, il en d\'ecoule 
\begin{align*}
y_{i+1} &= y_i - \lceil y_i/2 \rceil\\
&= \lfloor y_i/2 \rfloor\\
&\le y_i/2,
\end{align*}
puis on a $y_i \le n/2^i$. Il s'ensuit que $x_i = \lceil y_i/2 \rceil \le \lceil n/2^{i+1} \rceil$.
\end{exrev}
\end{description}
